...
export default function MemberPayments() {
  ...
  async function handlePay(option) {
    ...
    const result = await startCheckout({ description: option.label, amount: option.amount })
  ...
  return (
    <div className="space-y-8">
      <section>
        ...
        <p className="mt-1 text-sm text-white/50">Submit dues and event fees via secure Stripe Checkout.</p>
        ...
        <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-3">
          {DUE_OPTIONS.map((option) => (
            <div key={option.id} className="glass-panel rounded-xl p-5">
              ...
              <button type="button" onClick={() => handlePay(option)} className="btn-ember mt-4 w-full">
                Pay now
              </button>
              {status?.id === option.id && (
                <p className={`mt-2 text-xs ${status.ok === false ? 'text-blood-light' : 'text-white/50'}`}>{status.message}</p>
              )}
            </div>
          ))}
        </div>
