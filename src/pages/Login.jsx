    ...
    <div className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative flex min-h-screen items-center justify-center px-4 py-12">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="glass-panel space-y-5 rounded-2xl p-6 shadow-2xl shadow-black/40">
            ...
            <div className="space-y-3">
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-white/40">Email</label>
                <input
                  type="email"
                  required
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  placeholder={selected === 'admin' ? 'harris@noidols.org' : 'you@brotherhood.org'}
                  className="input-field"
                />
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-white/40">Password</label>
...
function RoleTile({ active, onClick, icon: Icon, title, subtitle }) {
  return (
    ...
    >
      ...
      <span className={`text-[10px] ${active ? 'text-ink/70' : 'text-white/40'}`}>{subtitle}</span>
    </button>
  )
}
