...
export default function Login() {
  const { firebaseUser, authLoading, authError, signInWithGoogle, signInWithGithub, signInWithMicrosoft, signInWithEmail, registerWithEmail, isFirebaseConfigured } = useAuth()
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      <div
        ...
        style={{ backgroundImage: "url('/posters/no-idols-season-session-man-doing-dua.jpg')" }}
      />
      ...
      <div className="relative flex min-h-screen items-center justify-center px-4 py-12">
        <div className="w-full max-w-md">
          <div className="mb-8 flex flex-col items-center text-center">
            ...
            <p className="mt-2 text-sm text-white/50">Brotherhood revival &middot; member portal &amp; goal tracker</p>
          </div>
          ...
          <div className="glass-panel space-y-5 rounded-2xl p-6 shadow-2xl shadow-black/40">
            {!isFirebaseConfigured && (
              <p className="rounded-md bg-amber-500/10 px-3 py-2 text-xs text-amber-400 ring-1 ring-amber-500/30">
                Firebase isn&apos;t configured yet. Set the VITE_FIREBASE_* env vars to enable sign-in.
              </p>
            )}

            <div className="space-y-2">
              <SsoButton onClick={signInWithGoogle} label="Continue with Google" mark={<GoogleMark />} />
              <SsoButton onClick={signInWithGithub} label="Continue with GitHub" mark={<GithubMark />} />
              <SsoButton onClick={signInWithMicrosoft} label="Continue with Microsoft" mark={<MicrosoftMark />} />
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              {authError && <p className="text-xs text-blood-light">{authError}</p>}

              <button type="submit" disabled={submitting} className="btn-ember w-full">
...
function GoogleMark() {
  return (
    <svg className="h-4 w-4" viewBox="0 0 24 24">
      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.07 5.07 0 0 1-2.2 3.33v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.11z" />
      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84A11 11 0 0 0 12 23z" />
      <path fill="#FBBC05" d="M5.84 14.1A6.6 6.6 0 0 1 5.5 12c0-.73.13-1.44.34-2.1V7.06H2.18A11 11 0 0 0 1 12c0 1.77.42 3.45 1.18 4.94z" />
