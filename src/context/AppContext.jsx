...
export function AppProvider({ children }) {
  ...
  const logCashPayment = useCallback(
    async ({ memberId, amount, type }) => {
      ...
      setMembers((prev) =>
        prev.map((m) =>
          m.id === memberId ? { ...m, paymentStatus: "Paid" } : m,
        ),
      );
    },
    [members],
    ...
    setMembers((prev) =>
      prev.map((m) =>
        m.id === memberId
          ? {
              ...m,
              sessionsAttended: Math.min(
                m.sessionsAttended + 1,
                m.sessionsTotal,
              ),
              attendanceRate: Math.round(
                (Math.min(m.sessionsAttended + 1, m.sessionsTotal) /
                  m.sessionsTotal) *
                  100,
              ),
            }
  ...
  const value = useMemo(
    () => ({
      ...
      payments,
      goals: currentMember
        ? goals.filter((g) => g.memberId === currentMember.id)
        : goals,
      rsvps,
      rsvp,
      logCashPayment,
